...
folio_setup_env_auto:
	...
	cat $(laravel_env_path)/.env | sed -e s/\{DB_USERNAME}/$(db_user)/g | tee $(laravel_env_path)/.env && \
	cat $(laravel_env_path)/.env | sed -e s/\{DB_PASSWORD}/$(db_password)/g | tee $(laravel_env_path)/.env \
	)
	@echo "##########################################"
	@echo ""